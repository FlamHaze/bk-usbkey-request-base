...
import {elementAddEvent, elementRemoveEvent, getElementsByClassName, removeElementsByName} from "./domExtends";
...
const FORM_REQUEST_TIMEOUT_FLAG = "BK_CROSS_FORM_REQUEST_FORM_REQUEST_TIMEOUT_FLAG";
...
export function addIframe(): string {
    ...
    if (formCalcEle) {
        const nowNumber = parseInt((formCalcEle as unknown as HTMLInputElement).value);
...
export function addFormCalc(): number {
    ...
    if (formCalcEle) {
        const nowNumber = parseInt((formCalcEle as unknown as HTMLInputElement).value);
        (formCalcEle as unknown as HTMLInputElement).value = (nowNumber + 1) as unknown as string;
        return nowNumber + 1;
    } else {
...
export function addForm(params: CrossSendData, formName: string) {
    ...
    form.setAttribute(FORM_REQUEST_TIMEOUT_FLAG, (params.timeOut as number).toString());
 ...
 * 具体操作.
 */
export function operate(errorJudgeFun: (jsonData: any, strData: string) => boolean = () => true, formName: string, isNoFirst: boolean = true): Promise<CrossResponseMap<CrossResponse>> | null {
 ...
 * 操作Form跨域表单
 */
export function operateForm(errorJudgeFun: (jsonData: any, strData: string) => boolean = () => false, formName: string, isNoFirst: boolean, result: CrossResponseMap<CrossResponse> = {length: 0}): Promise<CrossResponseMap<CrossResponse>> | null {
    ...
    return crossOperate(formName, iframe as unknown as HTMLIFrameElement, operateStateFlag, result, errorJudgeFun);
 ...
 * @param errorJudgeFun
 */
function crossOperate(formName: string, iframe: HTMLIFrameElement, operateStateFlag: HTMLDivElement, result: CrossResponseMap<CrossResponse>, errorJudgeFun?: (jsonData: any, strData: string) => boolean): Promise<CrossResponseMap<CrossResponse>> | null {
    ...
    if (formsLength == 1) {
        ...
        formsArray.sort((e1, e2) => {
            ...
            const e1Num = parseInt((e1 as unknown as HTMLFormElement).getAttribute("index") as string);
            const e2Num = parseInt((e2 as unknown as HTMLFormElement).getAttribute("index") as string);
        ...
        elementAddEvent(window, "message", (e) => {
            ...
            if (typeof errorJudgeFun === "function") {
                ...
                if (isErr) {
                    ...
                    elementRemoveEvent(window, "message");
                    document.body.removeChild(document.getElementById(OPERATE_STATE_ID) as Node);
                    reject(createCrossResponseError({
                        code: "CUSTOM_ERROR",
                        message: "响应数据处理错误！",
                        jsonData,
 ...
 * @param reject 错误回调
 */
const formSubmit = function (iframe: HTMLIFrameElement, formsArray: Array<HTMLFormElement>, resultLength: number, reject: (response?: any) => void): any {
        const form = formsArray.splice(0, 1);
        const nameSign = form[0].getAttribute(FORM_NAME_SIGN) as string;
        const strTimeOut: string = form[0].getAttribute(FORM_REQUEST_TIMEOUT_FLAG) as string;
        ...
        const timeOutId = startTimeOut(numberTimeOut, formName, nameSign, resultLength, resultLength + formsArray.length + 1, reject)
 ...
 * @param reject 错误回调
 */
const startTimeOut = function (timeOut: number, formName: string, nameSign: string, successLength: number, totalOperateLength: number, reject: (response?: any) => void): any {
